...
void CGLMesh::Render(GLenum mode)
{
	...
	for ( int i = 0; i < Mesh.nNumTriangles; i++ )
	{
		...
		sTriangle* pTriangle = &Mesh.pTriangles[i];

		glBegin(GL_TRIANGLES);							// Start met het teken van triangles
		for ( int j = 0; j < 3; j++ )
		{
			// Vraag een pointer naar de vertex op
			sVertex* pVertex = &Mesh.pVertices[pTriangle->nVertexIndices[j]];
...
void CGLMesh::DrawGizmo()
{
	...
	glBegin(GL_LINES);
	{
		...
		glVertex3f(min[0]-GIZMO_OFFSET, min[1]-GIZMO_OFFSET, min[2]-GIZMO_OFFSET);
		glVertex3f(max[0]+GIZMO_OFFSET, min[1]-GIZMO_OFFSET, min[2]-GIZMO_OFFSET);

		glVertex3f(min[0]-GIZMO_OFFSET, min[1]-GIZMO_OFFSET, min[2]-GIZMO_OFFSET);
		glVertex3f(min[0]-GIZMO_OFFSET, max[1]+GIZMO_OFFSET, min[2]-GIZMO_OFFSET);

		glVertex3f(min[0]-GIZMO_OFFSET, min[1]-GIZMO_OFFSET, min[2]-GIZMO_OFFSET);
		glVertex3f(min[0]-GIZMO_OFFSET, min[1]-GIZMO_OFFSET, max[2]+GIZMO_OFFSET);

		glColor3f(0.0f, 1.0f, 0.0f);
		glVertex3f(max[0]+GIZMO_OFFSET, max[1]+GIZMO_OFFSET, max[2]+GIZMO_OFFSET);
		glVertex3f(min[0]-GIZMO_OFFSET, max[1]+GIZMO_OFFSET, max[2]+GIZMO_OFFSET);

		glVertex3f(max[0]+GIZMO_OFFSET, max[1]+GIZMO_OFFSET, max[2]+GIZMO_OFFSET);
		glVertex3f(max[0]+GIZMO_OFFSET, min[1]-GIZMO_OFFSET, max[2]+GIZMO_OFFSET);

		glVertex3f(max[0]+GIZMO_OFFSET, max[1]+GIZMO_OFFSET, max[2]+GIZMO_OFFSET);
		glVertex3f(max[0]+GIZMO_OFFSET, max[1]+GIZMO_OFFSET, min[2]-GIZMO_OFFSET);
	}
	glEnd();
