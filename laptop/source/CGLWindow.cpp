...
void CGLWindow::SetSelect( SELECT_CALLBACK pfnSelect, VOID* pMain )
{
	this->m_pfnSelect = pfnSelect;
	this->m_pMain = pMain;
}
...
void CGLWindow::End()
{
	...
	glTranslatef(-0.55f, -0.35f, -1.0f);

	glRotatef(m_pCamera->GetXRotation(), 1.0f, 0.0f, 0.0f);
	glRotatef(m_pCamera->GetYRotation(), 0.0f, 1.0f, 0.0f);
	glRotatef(m_pCamera->GetZRotation(), 0.0f, 0.0f, 1.0f);

	glBegin(GL_LINES);

		// X-AS
		glColor3f(0.0f, 0.0f, 1.0f);
		glVertex3f(0, 0, 0); glVertex3f(0.05f, 0, 0);

		// Y-AS
		glColor3f(0.0f, 1.0f, 0.0f);
		glVertex3f(0, 0, 0); glVertex3f(0, 0.05f, 0);

		// Z-AS
		glColor3f(1.0f, 0.0f, 0.0f);
		glVertex3f(0, 0, 0); glVertex3f(0, 0, 0.05f);
	glEnd();
